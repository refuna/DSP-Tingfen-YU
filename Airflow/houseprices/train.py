...
def build_model(df: pd.DataFrame) -> int:
    score = {}
    y = df["SalePrice"]
    X = preprocess(df, is_training=True)
    ...
    joblib.dump(model, airflow_home_folder +'/models/LGBM_model.joblib')
    score['rmse'] = compute_rmsle(y_test, pred_test_y, 2)
    return score


if __name__ == '__main__':
    df = pd.read_csv('./dags/data/new_data/train.csv')
