...
@app.post("/single-prediction") # provide the address http://localhost:8000/single-prediction that will be used to call the API for single prediction
...
@app.post("/multi-predictions" ) # provide the address http://localhost:8000/multi-predictions that will be used to call the API for multiple predictions
...
async def predict(csv_file: UploadFile):
    ...
    predict_df = user_data_df.join(pd.DataFrame(multi_predictions, columns=['SalePrice']))
    # save the prediction in the database --> db.py
...
@app.get('/past-predictions', status_code=200) # provide the address http://localhost:8000/past-predictions that will be used to call the API for past predictions
