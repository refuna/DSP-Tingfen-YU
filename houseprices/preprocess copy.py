...
#         df_scaled = pd.DataFrame(sc.transform(
...
def preprocess(dataframe: pd.DataFrame, is_training: bool = False) -> pd.DataFrame:
    df = impute_missing_data(dataframe)
    continuous_features_df = preprocess_continuous_features(df)
    categorical_features_df = preprocess_categorical_features(df)
    return categorical_features_df.join(continuous_features_df)


def preprocess_continuous_features(df: pd.DataFrame) -> pd.DataFrame:
    continuous_columns = df.select_dtypes(include='number').columns.tolist()
    sc_path = 'models/sc.joblib'
...
def preprocess_categorical_features(df: pd.DataFrame) -> pd.DataFrame:
    get_ont_hot_encoder = get_OneHotEncoder(df)
    categorical_features_df = df_OneHotEncoder(df, get_ont_hot_encoder)
...
if __name__ == '__main__':
    ...
    result = preprocess(df_test[feature_columns])
    print(result)
